...
%caluclatind SINAD from ENOB:
ea_ML = sqrt(ML_params(1)^2 + ML_params(2)^2);
ea_LS = sqrt(LS_params_NORM(1)^2 + LS_params_NORM(2)^2);

LS_SINAD = 1.76 + 6.02*LS_ENOB + 20*log10(ea_ML / 1);
ML_SINAD = 1.76 + 6.02*ML_ENOB + 20*log10(ea_LS / 1);
...
try
    ...
    if (existings_index ~= 0) %existing result struct
        %Adding new results:
        testresults{existings_index,1}.LS.SINAD = LS_SINAD;
        testresults{existings_index,1}.LS.ENOB = LS_ENOB;                        
        testresults{existings_index,1}.ML.SINAD = ML_SINAD;
        testresults{existings_index,1}.ML.ENOB = ML_ENOB;                
    ...
    testresults{1,1}.LS.ENOB = LS_ENOB;        
    testresults{1,1}.ML.SINAD = ML_SINAD;
    testresults{1,1}.ML.ENOB = ML_ENOB;        
