...
%periódusból f/fs:
P.f = P.f/Nx;
%Szinusz el?jel korrekciója
P.B = (-1)*P.B;

%Fáziskorrekció:
est_amp = sqrt(P.A^2 + P.B^2);
est_phase = atan2(P.B,P.A);
est_phase = est_phase + 2*pi*P.f; %1 mintányival korrigáljuk a fázist: (0..N-1)*Ts helyett (1..N)*Ts
P.A = est_amp*cos(est_phase);
P.B = est_amp*sin(est_phase);
end
